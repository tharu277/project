...
const CENTER = [6.9271, 79.8612]; // Colombo / Default Center

// Auto-fit map to active bus markers
...
export default function Home() {
  ...
  // Fetch Live Active Buses every 5s
  useEffect(() => {
    const fetchBuses = () => {
  ...
  const handleSearchSubmit = (e) => {
    if (e.key === 'Enter') {
      navigate(`/tracking?search=${encodeURIComponent(search)}`);
    }
  };
    ...
    <div style={S.page}>
      ...
      <div style={S.hero}>
        <div style={S.heroLeft}>
          ...
          <h1 style={S.heroTitle}>Track Your Bus<br/> in Real Time!</h1>
          <p style={S.heroDesc}>
            Once the driver starts the trip, the live bus location is displayed
            on the map together with current speed, route information, and estimated arrival time (ETA).
          </p>
          <div style={S.heroBtns}>
            {!user && <Link to="/register" style={S.heroBtnPrimary}>✨ Create a Free Account</Link>}
            <a href="#map-section" style={S.heroBtnSecondary}>🗺 View Live Map ↓</a>
          </div>
        </div>
      ...
      {/* ── MAIN MAP + BUS LIST SECTION ── */}
      <div id="map-section" style={S.mainContent}>

        {/* LEFT PANEL: Live Buses */}
        <div style={S.leftPanel}>
          ...
          <div style={S.busScrollList}>
            ...
            {!loading && filtered.length === 0 && (
              <div style={S.emptyBox}>
                ...
                <div style={S.emptyTitle}>No Active Buses Available</div>
          ...
          {/* Selected Bus Floating Box */}
          {selected && (
            <div style={S.selectedDetail}>
              <div style={S.sdTitle}>📋 {selected.busNumber} Details</div>
              <div style={S.sdGrid}>
                <div style={S.sdItem}>
                  <div style={S.sdLabel}>Route</div>
                  <div style={S.sdVal}>{selected.routeId?.routeNumber || '—'}</div>
                </div>
                <div style={S.sdItem}>
                  <div style={S.sdLabel}>Speed</div>
                  <div style={{ ...S.sdVal, color: '#1d4ed8' }}>{Math.round(selected.speed || 0)} km/h</div>
                </div>
                <div style={S.sdItem}>
                  <div style={S.sdLabel}>From</div>
                  <div style={S.sdVal}>{selected.routeId?.origin || '—'}</div>
                </div>
                <div style={S.sdItem}>
                  <div style={S.sdLabel}>To</div>
                  <div style={S.sdVal}>{selected.routeId?.destination || '—'}</div>
                </div>
              </div>
        ...
        <div style={S.mapWrap}>
          ...
          <div style={S.mapOverlayTop}>
            ...
            <div style={S.mapLiveBadge}><span style={S.liveDot}></span> Live Feed</div>
          </div>
        </div>
      ...
      <style>{`
        ...
        #map-section { scroll-margin-top: 70px; }
...
const S = {
  ...
  searchWrap: { display: 'flex', alignItems: 'center', gap: '8px', background: '#f1f5f9', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '0 14px', height: '42px', width: '100%', maxWidth: '420px' },
